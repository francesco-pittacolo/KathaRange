...
export PATH=$PATH:$JAVA_HOME/bin:$INSTALLATION_DIR/bin
...
# ---------------------------
# Function to run commands as wazuh-indexer user
# ---------------------------
run_as_wazuh_indexer() {
    if [[ "$(id -u)" == "0" ]]; then
        # If running as root, switch to wazuh-indexer user
        exec su -s /bin/bash wazuh-indexer -c "$*"
    else
        # If already non-root, run command directly
        exec "$@"
    fi
}
...
# ---------------------------
source /usr/share/wazuh-indexer/bin/opensearch-env-from-file || true

# ---------------------------
# Create keystore if INDEXER_PASSWORD is set
# ---------------------------
if [[ -n "$INDEXER_PASSWORD" ]]; then
    [[ -f /usr/share/wazuh-indexer/opensearch.keystore ]] || run_as_wazuh_indexer opensearch-keystore create
    if ! run_as_wazuh_indexer opensearch-keystore has-passwd --silent ; then
        if ! run_as_wazuh_indexer opensearch-keystore list | grep -q '^bootstrap.password$'; then
            echo "$INDEXER_PASSWORD" | run_as_wazuh_indexer opensearch-keystore add -x 'bootstrap.password'
        fi
    fi
fi

# ---------------------------
# Fix ownership for bind-mounted or copied directories
# ---------------------------
if [[ "$(id -u)" == "0" ]]; then
    chown -R wazuh-indexer:wazuh-indexer /certs /usr/share/wazuh-indexer/data /usr/share/wazuh-indexer/logs || true
fi

# ---------------------------
# Function to dynamically build OpenSearch configuration
# ---------------------------
    ...
    for opensearch_var in ${opensearch_vars[*]}; do
        ...
        if [[ -n $value ]]; then
            if grep -q $opensearch_var $OPENSEARCH_PATH_CONF/opensearch.yml; then
                lineNum="$(grep -n "$opensearch_var" $OPENSEARCH_PATH_CONF/opensearch.yml | head -n1 | cut -d: -f1)"
                sed -i "${lineNum}d" $OPENSEARCH_PATH_CONF/opensearch.yml
            fi
            echo "${opensearch_var}: ${value}" >> $OPENSEARCH_PATH_CONF/opensearch.yml
        fi
    done
}

# Build OpenSearch configuration from environment variables
buildOpensearchConfig
...
if [[ "$1" == "opensearchwrapper" ]]; then
    # Run OpenSearch as wazuh-indexer user, passing keystore password
    run_as_wazuh_indexer /usr/share/wazuh-indexer/bin/opensearch <<<"$KEYSTORE_PASSWORD"
else
    # Run any other provided command as wazuh-indexer
    run_as_wazuh_indexer "$@"
fi
